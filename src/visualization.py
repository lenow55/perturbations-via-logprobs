...
def create_highlighted_text_html(data: list[TokenEntropy], config: AppSettings) -> str:
    """
...
def create_plotly_chart(
    data: list[TokenEntropy], title: str, config: AppSettings
) -> str:
...
def generate_full_report(
    scenario_results: list[tuple[str, ScenarioResult]],
    filename: str,
    config: AppSettings,
):
    ...
    <p>Красным цветом выделены токены, где модель испытывала высокую неуверенность (Internal Confusion).</p>
    """

    for idx, sc in scenario_results:
        logger.info(f"Обработка сценария: {idx}...")

        chart_html = create_plotly_chart(sc["logprobs"], sc["name"], config)
        text_html = create_highlighted_text_html(sc["logprobs"], config)

        full_html += f"""
        <div class="section">
            <h2>Сценарий: {sc["name"]}</h2>
            <p><strong>Prompt:</strong> <em>{sc["text"][:100]}...</em></p>
            {chart_html}
